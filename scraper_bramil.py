...
CSV_PATH      = os.path.join(DATA_DIR, "produtos_bramil.csv")
ALERTA_PATH   = os.path.join(DATA_DIR, "alertas_bramil.txt")
SNAPSHOT_PATH = os.path.join(DATA_DIR, "_snapshot_bramil.json")
...
def salvar(produtos_unicos, alertas):
    ...
    if alertas:
        ...
        relatorio = f"ALERTAS BRAMIL - {data_exibicao}\n{'=' * 44}\nTotal: {len(alertas)}\n\n"
        for a in alertas:
            relatorio += f"{a['tipo']}\nProduto: {a['produto']}\n{a['detalhes']}\n\n"
